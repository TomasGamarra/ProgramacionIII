...
public class AdjListGraph<T> implements Graph<T> {
	...
	//Lo implemente yo
	public void connectBoth (Vertex<T> origin,Vertex<T> destination,int weight) {
		if (this.belongs(origin) && this.belongs(destination)) {
			((AdjListVertex<T>) origin).connect(destination, weight);
			((AdjListVertex<T>) destination).connect(origin, weight);
		}
	}
	public void connectBoth(Vertex<T> v1, Vertex <T> v2) {
        if (this.belongs(v1) && this.belongs(v2)) {
            ((AdjListVertex<T>) v1).connect(v2);
            ((AdjListVertex<T>) v2).connect(v1);
        }
    }
